...
### 5. Modul Assembly & Reverse Engineering (`assembly/`)
...
### 7. Modul Steganography & Audio (`stego/`)
...
| **Reversing** | `ghidraanalyze status` | Cek jembatan live ke Ghidra GUI Windows |
| **Reversing** | `ghidraanalyze goto <addr>` | Pindahkan layar & kursor Ghidra GUI |
| **Reversing** | `ghidraanalyze current` | Decompile fungsi di posisi kursor Ghidra |
| **Reversing** | `exefix <dumped.exe> -o <out>` | Realign section header PE dump memori |
| **Reversing** | `exedoctor <file.exe>` | Diagnosa kerusakan PE header & sections |
| **Reversing** | `ghidraguide <file.exe>` | Blueprint analisis Ghidra & checklist Writeup |
| **Reversing** | `elfdoctor <binary.elf>` | Diagnosa Linux ELF: stripped symbols, UPX |
| **Reversing** | `disasm <payload.bin> -a x64` | Disassemble shellcode x64/x86/ARM |
...
| **Stego** | `stegoscan <image.png> --fix-height` | Perbaiki PNG IHDR height mismatch (CRC) |
| **Stego** | `stegodiff <img1> <img2>` | Auto-bruteforce shift 2D & residual delta |
| **Stego** | `stegocrack <secret.jpg> -w <dict>` | Cracking password Steghide & OutGuess |
| **Stego** | `stegoaudio <sound.wav>` | Visualisasi spektrogram suara FFT resolusi tinggi |
